...
physical_ports = {
    ...
    "acquire":{
        ...
        "port_delay": (120+200)*1e-9, # Because readout pulse shape is gussian square, for eliminating the rising edge.
        "port_mode": None,
        "delay_signal": None, # Global delay for the signal line, implemented by adjusting the waveform.
        "threshold": None,
        "added_outputs": None, # Added outputs to the signal line's physical channel port. (RTR option enabled)
...
qubits_parameters = {
    ...
    "q2": {
        ...
        "ge_frequency" : 4.567e9 + 0.183e6,
        "ef_frequency" : 4.567e9, #4.275e9, #4.3865e9,
        "readout_frequency" : 7.66457e9, # 7.6782e9 - 0.7e6, 
        ### readout parameters ###
        "readout_amp" : 0.23, #0.25, #0.18,
        'readout_pulse_length': 1400e-9,
        "readout_integration_length": (1200)*1e-9,
        "readout_integration_amp": 1,

        "drachma_readout_pulse_length": 1400e-9,
        "drachma_readout_integration_length": (1240)*1e-9,
        "drachma_readout_amp": 0.2,

        "readout_phase": (-70) * np.pi/180,
        ### drive parameters ###
        "drive_amp" : 0.356, 
        "drive_pulse_length": 64e-9,
        "drive_beta": 0.015,
        ...
        ### extra parameters ###
        "reset_delay_length": 300e-6,
        "readout_kappa": 2*np.pi*0.5e6,
        "readout_chi": 2*np.pi*1.2e6,
...
cavity_parameters = {
    ...
    'm0': { # mm2 - qubit 4.394GHz
        "mode_frequency": 5.152159e9 + 0.025e6, #5.152209e9 - 0.1e6, # center of g and e freq
        "cavity_drive_length": 40e-9, # this should be as short as possible to cover wide frequency range
        "cavity_drive_amp": 0.0452*3, #(this is the maximum amplitude range, and this goes to amp of experiment)
        "alpha_1_cavity_drive_amp": 0.0452, # 1 when it is not found yet. (This is amp when photon = 1) (this should be same with scaling_factor in disp_amp_calibration_parity

        "cond_disp_pulse_length": 200e-9, #200e-9,
        "cond_disp_pulse_amp": 0.0617, #0.0446j, # (this is the maximum amplitude range, and this goes to amp of experiment)
        "alpha_1_CNOD_amp": 0.0617, # 1 when it is not found yet. (this should be same with scaling_factor in CNOD calibration)
        ...
        "geophase_correction_coeff": 0.0207/2, # after calibrating geometric phase.

        "sideband_length": 1e-6,
        "sideband_chunk_length": 1e-6,
        "sideband_rise_fall_length": 120e-9,

        # "sideband_frequency_l" : 20e6, #30e6, # input unit is Hz
        # "sideband_frequency_h" : 20e6, #30e6,
        # "sideband_amp_l" : 0.1,
        # "sideband_amp_h" : 0.1,
        # "sideband_phase" : 0, # in radians
        # "sideband_att_l" : 203, #448, # The larger value, the smaller attenuation
        # "sideband_att_h" : 108, #239, # The larger value, the smaller attenuation
        # "sideband_extra_phase": 0, # in radians

        # "sideband_frequency_l" : 30e6, # input unit is Hz
        # "sideband_frequency_h" : 30e6,
        # "sideband_amp_l" : 0.1,
        # "sideband_amp_h" : 0.1,
        # "sideband_phase" : 0, # in radians
        # "sideband_att_l" : 231,  # The larger value, the smaller attenuation
        # "sideband_att_h" : 92,  # The larger value, the smaller attenuation
        # "sideband_extra_phase": 0, # in radians
        ...
        "sideband_amp_h" : 0.1,
        "sideband_phase" : 0, # in radians
        "sideband_att_l" : 575.17,  # The larger value, the smaller attenuation
        "sideband_att_h" : 527.4,  # The larger value, the smaller attenuation
        "sideband_extra_phase": 0, # in radians
...
# %% 
my_run.nopi_pi(average_exponent=12, phase = 110, readout_pulse_type="gaussian_square",
...
my_run.single_shot_nopi_pi(npts_exponent = 12, phase = 110, readout_pulse_type="gaussian_square",
                            readout_weighting_type ="cavity_trajectory", is_plot_simulation=False)
my_run.plot_nopi_pi(npts = 108)

# %%
my_run.nopi_pi(average_exponent=12, phase = 100, readout_pulse_type="drachma",
                            readout_weighting_type ="drachma", is_plot_simulation=False) 
## readout weighting is not important for above experiment ; result is obtained through RAW data
my_run.single_shot_nopi_pi(npts_exponent = 12, phase = 100, readout_pulse_type="drachma",
                            readout_weighting_type ="drachma", is_plot_simulation=False)
...
my_run.T1(average_exponent=10, duration = 100e-6, npts = 51, is_plot_simulation=False)
my_run.plot_T1()

# In[]
my_run.qubits_parameters['q0']["pi2_amp"] = 0.271

my_run.Pi2_cal(average_exponent=11, start = 0, npts = 12, is_plot_simulation=False)
my_run.plot_Pi2_cal()

# In[]
my_run.qubits_parameters['q0']["pi_amp"] = 0.542
...
my_run.Ramsey(is_echo = False, n_pi_pulse=1, # for CP or CPMG
              ...
              detuning = 0.2e6, 
              average_exponent=10, duration = 20e-6, npts = 80,
...
my_run.Ramsey_with_photon(is_echo = False,
            ...
            average_exponent=10, duration = 10e-6, npts = 100, #duration/npts = integer
            steady_time = 1e-6,
...
my_run.Ramsey_amp_sweep(is_echo = False, detuning = 0.2e6, 
              ...
              sweep_start = 0.0, sweep_stop = 0.1, sweep_npts = 11, # sweep_start = -np.pi/16, sweep_stop = np.pi/16, sweep_npts = 11,
              ...
              kappa = 0.013, chi = 0.125, # in MHz unit # chi = chi_ge/2
              steady_time = 5e-6,
...
# In[]
my_run.qubits_parameters['q0']["rabi_drive_amp"] = 0.1658

my_run.Rabi_length(average_exponent=8, duration = 1e-6, start_time = 1e-6, npts = 200,
...
my_run.cavity_parameters['m0']['sideband_phase'] = 2.12

my_run.qubits_parameters['q0']["rabi_drive_amp"] = 0.1658

my_run.Rabi_length_with_photon(average_exponent=8, duration = 1e-6, start_time = 1e-6, npts = 200,
                   detuning_freq = -0.52e6, # detuned freq for qubit drive
...
my_run.qubits_parameters['q0']["rabi_drive_amp"] = 0.1786

my_run.Rabi_length_spin_locking(average_exponent = 8, duration = 1e-6, start_time = 1e-6,
                                detuning_freq = -0.0e6, # detuned freq for qubit drive
                                npts = 200, rabi_phase = 0, is_init_pi2=False, init_pi2_phase=0,
                                is_sideband_drive= True, steady_time = 5e-6, 
                                is_plot_simulation = False)
my_run.plot_Rabi_length_spin_locking(is_xyz_plot = False, is_purity_plot = True)

my_run.cavity_parameters['m0']['cavity_drive_amp'] = 0.1
my_run.cavity_parameters['m0']['sideband_amp_l'] = 0.2
my_run.cavity_parameters['m0']['sideband_amp_h'] = 0.2
my_run.cavity_parameters['m0']['sideband_phase'] = 2.12 + 1.57

my_run.qubits_parameters['q0']["rabi_drive_amp"] = 0.1786

my_run.Rabi_length_spin_locking(average_exponent = 8, duration = 5e-6, start_time = 1e-6,
                                detuning_freq = -0.51e6, # detuned freq for qubit drive
                                npts = 1000, rabi_phase = 0, is_init_pi2=False, init_pi2_phase=0,
                                is_sideband_drive= True, steady_time = 5e-6, 
                                is_plot_simulation = False)
...
my_run.cavity_T1(average_exponent=10, start = 0e-6, duration = 100e-6, npts = 51, is_plot_simulation=False)
...
my_run.cavity_pi_nopi(average_exponent=9, freq_start = -0.2e6, freq_stop = 0.2e6, 
                      freq_npts = 80, 
                      is_qubit2 = False, 
                      qubit2 = 1,
                      auto_chunking=True,
...
my_run.cavity_parameters['m0']['cavity_drive_amp'] = 0.0452
...
my_run.plot_CNOD_calibration(scaling_factor= 0.0617)

# In[]
my_run.cavity_parameters['m0']['cond_disp_pulse_amp'] = 0.0617*5
...
my_run.cavity_parameters['m0']['cavity_drive_amp'] = 0.4 # sweep variable
...
my_run.cavity_parameters['m0']['alpha_1_CNOD_amp'] = 0.033
...
my_run.cavity_parameters['m0']["sideband_amp_h"] = 0.2
my_run.cavity_parameters['m0']["sideband_phase"] = 0
my_run.cavity_parameters['m0']["sideband_att_l"] = 174.2
my_run.cavity_parameters['m0']["sideband_att_h"] = 126
...
my_run.calibrate_sideband_pulse_phase(average_exponent=10, sidebands_pulse_length= 10e-6, rabi_pulse_length= 3e-6,
                                    qubit_drive_detuning_freq= -0.52e6,
...
my_run.cavity_parameters['m0']['cavity_drive_amp'] = 0.0452*4 # sweep variable for wigner
my_run.cavity_parameters['m0']['alpha_1_cavity_drive_amp'] = 0.0452
my_run.cavity_parameters['m0']['cond_disp_pulse_amp'] = 0.0618 * 3
my_run.cavity_parameters['m0']["alpha_1_CNOD_amp"] = 0.0618

my_run.cavity_parameters['m0']['sideband_amp_l'] = 0.1
my_run.cavity_parameters['m0']['sideband_amp_h'] = 0.1
my_run.cavity_parameters['m0']['sideband_phase'] = 0
my_run.cavity_parameters['m0']["steady_time"] = 1e-6

# 'cavity_drive_amp' * amplitude에 해당하는 displaced state 형성

my_run.characteristic_function_1D(average_exponent=10, npts = 61, amp_range = 1j, amplitude = 0.25,
                                  acquire_delay = 120e-9, 
                                  alpha = 1, # for cat state 
                                  beta = 1, # for cat state
                                  qubit_phase = 0, chunk_count = 61,
...
my_run.cavity_parameters['m0']['cavity_drive_amp'] = 0.0452*4 # sweep variable for wigner
my_run.cavity_parameters['m0']['alpha_1_cavity_drive_amp'] = 0.0452
my_run.cavity_parameters['m0']['cond_disp_pulse_amp'] = 0.0617 * 10
my_run.cavity_parameters['m0']["alpha_1_CNOD_amp"] = 0.0617

# 'cavity_drive_amp' * amplitude에 해당하는 displaced state 형성
# 'cavity_drive_amp'/(2*'alpha_1_cavity_drive_amp') 사이즈에 해당하는 Re-Im plane plot in case of wigner

my_run.wigner_characteristic_function_2D(average_exponent=11, npts_x = 31, npts_y = 31, amplitude = 0.375, #amplitude for coherent state
                          is_wigner_function= False,
                          is_coherent_state = False, is_schrodinger_cat_state= False, 
                          is_schrodinger_cat_state_2=False,
                          is_cat_state=False,
...
# In[]
psi_init = qt.coherent(N = 30, alpha = 1.5)  + qt.coherent(N = 30, alpha = -1.5)
