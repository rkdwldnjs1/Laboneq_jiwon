...
from laboneq_applications.analysis.resonator_spectroscopy import analysis_workflow #, analysis_workflow_direct_CR
...
if TYPE_CHECKING:
    from laboneq.dsl.quantum import TransmonParameters #, TransmonParameters_direct_CR
    from laboneq.dsl.quantum.qpu import QPU
    from laboneq.dsl.quantum.quantum_element import QuantumElement #, QuantumElement_direct_CR
...
'''
@workflow.workflow(name="resonator_spectroscopy_direct_CR")
def experiment_workflow_direct_CR(
    session: Session,
    qpu: QPU,
    qubit: QuantumElement_direct_CR,
    frequencies: ArrayLike,
    temporary_parameters: dict[str, dict | TransmonParameters_direct_CR] | None = None,
    options: TuneUpWorkflowOptions | None = None,
) -> None:
    """The Resonator Spectroscopy Workflow.
    ...
    Example:
        ...
        ```
    """
    qubit = temporary_modify_direct_CR(qubit, temporary_parameters)

    exp = create_experiment_direct_CR(
        qpu,
        qubit,
        frequencies=frequencies,
    )
    ...
    result = run_experiment(session, compiled_exp)
    with workflow.if_(options.do_analysis):
        analysis_results = analysis_workflow_direct_CR(result, qubit, frequencies)
        qubit_parameters = analysis_results.output
        with workflow.if_(options.update):
            update_qubits(qpu, qubit_parameters["new_parameter_values"])
    workflow.return_(result)
...
@workflow.task
@dsl.qubit_experiment
def create_experiment_direct_CR(
    qpu: QPU,
    qubit: QuantumElement_direct_CR,
    frequencies: ArrayLike,
    options: ResonatorSpectroscopyExperimentOptions | None = None,
