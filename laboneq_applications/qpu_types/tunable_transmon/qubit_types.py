...
from laboneq.dsl.quantum import (
    QuantumElement,
    # QuantumElement_direct_CR,
    TransmonParameters,
    # TransmonParameters_direct_CR,
)
from laboneq.dsl.quantum.quantum_element import SignalType #, SignalType_direct_CR
...
@classformatter
@dataclass
class TunableTransmonQubitParameters_direct_CR(TransmonParameters_direct_CR):
    """Qubit parameters for `TunableTransmonQubit_CR` instances.
    qubit_name: int = 0
        The number-label of the qubit.
    ...
    Attributes:
        ...
        # Note: The direct CR Method is used (instead of the echoed CR method)
        # Added according to number_of_qubits
        CR_parameters: dict = {
            "CR_cq0_tq1_parameters": dict = {
                                        CR_drive_amplitude:
                                            Amplitude for a CR pulse on the target qubit transition.
                                        CR_target_cancellation_ratio:
                                            Ratio of (amplitude of the CR pulse) / (amplitude of the cancellation pulse).
                                        CR_drive_length:
                                            Length of e-f transition drive pulses (seconds).
                                        CR_target_asymmetric_rotaty_amplitude:
                                            Pulse parameters for e-f transition drive pulses.
                                            },
            "CR_cq1_tq0_parameters": dict = {...},
            ...
                                }
    ...
    # CR drive pulse parameters

    CR_parameters: dict = field(
...
@classformatter
@dataclass(init=False, repr=True, eq=False)
class TunableTransmonQubit_direct_CR(QuantumElement_direct_CR):
    """A class for a superconducting, flux-tuneable Transmon Qubit."""

    TRANSITIONS = ("ge", "ef", "cr")

    parameters: TunableTransmonQubitParameters_direct_CR

    def __init__(
        self,
        uid: str | None = None,
        signals: dict[str, LogicalSignal] | None = None,
        parameters: TunableTransmonQubitParameters_direct_CR | dict[str, Any] | None = None,
    ):
        """Initializes a new Transmon Qubit.
        ...
        """
        if parameters is None:
            self.parameters = TunableTransmonQubitParameters_direct_CR()
        elif isinstance(parameters, dict):
            self.parameters = TunableTransmonQubitParameters_direct_CR(**parameters)
        else:
            self.parameters = parameters
        super().__init__(uid=uid, signals=signals)
    ...
    def transition_parameters(self, transition: str | None = None) -> tuple[str, dict]:
        ...
        Arguments:
            transition:
                The transition to return parameters for. May be `None`,
                `"ge"`, `"ef"` or `"CR_..."`. `None` defaults to `"ge"`.
        ...
        Raises:
            ValueError:
                If the transition is not `None`, `"ge"`, `"ef"` or `"CR_..."`.
        """
        if transition is None:
            transition = "ge"
        if transition not in self.TRANSITIONS:
            raise ValueError(
                f"Transition {transition!r} is not one of None, 'ge', 'ef', or 'CR_...'.",
            )
        if transition == "ge":
            line = "drive"
            ...
            param_keys = ["amplitude_pi", "amplitude_pi2", "length", "pulse"]
            params = {
                        k: getattr(self.parameters, f"ef_drive_{k}") for k in param_keys
                        }
        ...
        elif transition[0:3] == "CR_":
            line_CR = "drive_line_{transition}"
            line_CR_active_cancellation = "drive_line_{transition}_active_cancellation"
            line_CR_asymmetric_rotary = "drive_line_{transition}_asymmetric_rotary"
            line = [line_CR, line_CR_active_cancellation, line_CR_asymmetric_rotary]
            
            param_keys = ["CR_drive_amplitude", "CR_target_cancellation_ratio", "CR_drive_length", "CR_target_asymmetric_rotaty_amplitude", "CR_drive_pulse"]
            params = {
                        k: getattr(self.parameters[f"{transition}_parameters"], k) for k in param_keys
                        }
    ...
    def from_logical_signal_group(
        ...
        uid: str,
        lsg: LogicalSignalGroup,
        parameters: TransmonParameters_direct_CR | dict[str, Any] | None = None,
    ) -> TunableTransmonQubit_direct_CR:
        """TunableTransmonQubit_direct_CR from logical signal group.

        Args:
            uid: A unique identifier.
            lsg: Logical signal group.
                TunableTransmonQubit_direct_CR understands the following signal line names:

                    - drive: 'drive', 'drive_line'
                    - drive_ef: 'drive_ef', 'drive_line_ef'
                    - drive_CR...: 'drive_CR...', 'drive_line_CR...', 'drive_line_CR..._active_cancellation', 'drive_line_CR..._asymmetric_rotary'
                    - measure: 'measure', 'measure_line'
                    - acquire: 'acquire', 'acquire_line'
                    - flux: 'flux', 'flux_line'
        ...
        """
        signal_type_map = {
            SignalType_direct_CR.DRIVE: ["drive", "drive_line"],
            SignalType_direct_CR.DRIVE_EF: ["drive_ef", "drive_line_ef"],
            SignalType_direct_CR.MEASURE: ["measure", "measure_line"],
            SignalType_direct_CR.ACQUIRE: ["acquire", "acquire_line"],
            SignalType_direct_CR.FLUX: ["flux", "flux_line"],
        }

        for i in range(len(dir(SignalType_direct_CR))):
            if "CR" in dir(SignalType_direct_CR)[i]:
                if int(dir(SignalType_direct_CR)[i][11])<parameters.number_of_qubits and int(dir(SignalType_direct_CR)[i][15])<parameters.number_of_qubits:
                    if parameters.qubit_name == int(dir(SignalType_direct_CR)[i][11]) or parameters.qubit_name == int(dir(SignalType_direct_CR)[i][15]):
                        signal_type_map.update(
                            {
                                SignalType_direct_CR.__dict__[dir(SignalType_direct_CR)[i]]: [
                                    f"drive_CR_cq{dir(SignalType_direct_CR)[i][11]}_tq{dir(SignalType_direct_CR)[i][15]}",
                                    f"drive_line_CR_cq{dir(SignalType_direct_CR)[i][11]}_tq{dir(SignalType_direct_CR)[i][15]}",
                                ]
                            }
                        )
                        signal_type_map.update(
                            {
                                SignalType_direct_CR.__dict__[dir(SignalType_direct_CR)[i]]: [
                                    f"drive_CR_cq{dir(SignalType_direct_CR)[i][11]}_tq{dir(SignalType_direct_CR)[i][15]}_active_cancellation",
                                    f"drive_line_CR_cq{dir(SignalType_direct_CR)[i][11]}_tq{dir(SignalType_direct_CR)[i][15]}_active_cancellation",
                                ]
                            }
                        )
                        signal_type_map.update(
                            {
                                SignalType_direct_CR.__dict__[dir(SignalType_direct_CR)[i]]: [
                                    f"drive_CR_cq{dir(SignalType_direct_CR)[i][11]}_tq{dir(SignalType_direct_CR)[i][15]}_asymmetric_rotary",
                                    f"drive_line_CR_cq{dir(SignalType_direct_CR)[i][11]}_tq{dir(SignalType_direct_CR)[i][15]}_asymmetric_rotary",
                                ]
                            }
                        )

        # print(signal_type_map)
        if parameters is None:
            parameters = TransmonParameters_direct_CR()
        elif isinstance(parameters, dict):
            parameters = TransmonParameters_direct_CR(**parameters)
        return cls._from_logical_signal_group(
            uid=uid,
            lsg=lsg,
            parameters=parameters,
