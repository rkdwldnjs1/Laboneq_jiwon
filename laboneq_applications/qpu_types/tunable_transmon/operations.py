...
from .qubit_types import TunableTransmonQubit #, TunableTransmonQubit_direct_CR
...
# ------------------------------------------------------------------ ADD : 12/30/2024 ------------------------------------------------------------------
'''
class TunableTransmonOperations_direct_CR(dsl.QuantumOperations):
    """Operations for TunableTransmonQubits."""

    QUBIT_TYPES = TunableTransmonQubit_direct_CR
    ...
    @dsl.quantum_operation
    def barrier(self, q: TunableTransmonQubit_direct_CR) -> None:
        """Add a barrier on all the qubit signals.
    ...
    @dsl.quantum_operation
    def delay(self, q: TunableTransmonQubit_direct_CR, time: float) -> None:
        """Add a delay on the qubit drive signal.
    ...
    def set_frequency(
        self,
        q: TunableTransmonQubit_direct_CR,
        frequency: float | SweepParameter,
        *,
        transition: str | None = None,
        readout: bool = False,
    ...
    @dsl.quantum_operation                                               # ADD : 12/30/2024
    def CR(
        self,
        cq: TunableTransmonQubit_direct_CR,
        tq: TunableTransmonQubit_direct_CR,
        amplitude: float | SweepParameter | None = None,
        length: float | SweepParameter | None = None,
        pulse: dict | None = None,
    ) -> None:
        ...
        """
        drive_line, params = q.transition_parameters(transition)
        # drive_line : line_CR, line_CR_active_cancellation, line_CR_asymmetric_rotary
        # params keys : "CR_drive_amplitude", "CR_target_cancellation_ratio", "CR_drive_length", "CR_target_asymmetric_rotaty_amplitude", "CR_drive_pulse"
        line_CR, line_CR_active_cancellation, line_CR_asymmetric_rotary = drive_line
        
        CR_drive_pulse = dsl.create_pulse(params["CR_drive_pulse"], pulse, name="CR_drive_pulse")
        CR_active_cancellation_pulse = dsl.create_pulse(params["CR_drive_pulse"], pulse, name="CR_target_cancellation_pulse")
        CR_asymmetric_rotary_pulse = dsl.create_pulse({"function": "sampled_pulse",
                                                       "samples": np.concatenate([np.linspace(1,1,length//2), np.linspace(-1,-1,length//2)]),},
                                                       pulse, 
                                                       name="CR_target_asymmetric_rotary_pulse"
                                                       ) # fix a shape of asymmetric rotary pulse

        dsl.play(
            q.signals[line_CR],
            amplitude=CR_drive_amplitude,
            length=length,
            pulse=CR_drive_pulse,
        )
        dsl.play(
            q.signals[line_CR_active_cancellation],
            amplitude=CR_target_cancellation_ratio * CR_drive_amplitude,
            length=length,
            pulse=CR_active_cancellation_pulse,
        )
        dsl.play(
            q.signals[line_CR_asymmetric_rotary],
            amplitude=CR_target_asymmetric_rotaty_amplitude,
            length=length,
            pulse=CR_asymmetric_rotary_pulse,
        )
    ...
    @dsl.quantum_operation
    def z90(self, q: TunableTransmonQubit_direct_CR, transition: str | None = None) -> None:
        """Rotate the qubit by 90 degrees about the Z-axis.
    ...
    @dsl.quantum_operation
    def z180(self, q: TunableTransmonQubit_direct_CR, transition: str | None = None) -> None:
        """Rotate the qubit by 180 degrees about the Z-axis.
    ...
    def active_reset(
        ...
        Arguments:
            ...
            measure_section_length:
                ...
                Default: None.
        """
        if isinstance(qubits, TunableTransmonQubit_direct_CR):
            qubits = [qubits]
