...
@workflow.workflow
def analysis_workflow_direct_CR(
    result: RunExperimentResults,
    ...
    Example:
        ...
        ```
    """
    processed_data_dict = calculate_signal_magnitude_and_phase_direct_CR(
        qubit, result, frequencies
    )
    fit_result = fit_data_direct_CR(processed_data_dict)
    qubit_parameters = extract_qubit_parameters_direct_CR(qubit, processed_data_dict, fit_result)
    with workflow.if_(options.do_plotting):
        with workflow.if_(options.do_raw_data_plotting):
            plot_raw_complex_data_1d(
                qubit,
        ...
        with workflow.if_(options.do_plotting_magnitude_phase):
            plot_magnitude_phase_direct_CR(
                qubit, processed_data_dict, fit_result, qubit_parameters
            )
        with workflow.if_(options.do_plotting_real_imaginary):
            plot_real_imaginary_direct_CR(qubit, result)
    workflow.return_(qubit_parameters)


@workflow.task
def calculate_signal_magnitude_and_phase_direct_CR(
    qubit: QuantumElement_direct_CR,
    result: RunExperimentResults,
    frequencies: ArrayLike,
) -> dict[str, ArrayLike]:
...
@workflow.task
def extract_qubit_parameters_direct_CR(
    qubit: QuantumElement_direct_CR,
    processed_data_dict: dict[str, ArrayLike],
    fit_result: lmfit.model.ModelResult | None,
    options: ResonatorSpectroscopyAnalysisOptions | None = None,
...
@workflow.task
def plot_magnitude_phase_direct_CR(
    qubit: QuantumElement_direct_CR,
    processed_data_dict: dict[str, ArrayLike],
    fit_result: lmfit.model.ModelResult | None,
    qubit_parameters: dict[
...
@workflow.task
def plot_real_imaginary_direct_CR(
    qubit: QuantumElement_direct_CR,
    result: RunExperimentResults,
    options: ResonatorSpectroscopyAnalysisOptions | None = None,
) -> mpl.figure.Figure | None:
