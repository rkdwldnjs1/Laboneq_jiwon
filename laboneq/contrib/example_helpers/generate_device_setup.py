...
from laboneq.contrib.example_helpers.example_notebook_helper import (
    create_dummy_transmon, create_dummy_transmon_cr, create_dummy_transmon_mult,
    generate_dummy_transmon_parameters, generate_dummy_transmon_cr_parameters, generate_dummy_transmon_mult_parameters
)
...
def generate_device_setup(
    ...
    memory_modes = [f"m{it}" for it in range(number_memory_modes)]
    ...
    if not readout_only:
        ...
        for qubit in qubits:
            ...
            if cr_drive_lines:
                device_setup.add_connections(
                    current_drive["device"],
                    create_connection(
                        to_signal=f"{qubit}/control_drive_line", ports=current_drive["port"]
                    ),
                    create_connection(
                        to_signal=f"{qubit}/target_drive_line_1", ports=current_drive["port"]
                    ),
                    create_connection(
                        to_signal=f"{qubit}/target_drive_line_2", ports=current_drive["port"]
                    ),
                )
            drive_index += 1
        ...
        for memory_mode in memory_modes:
            ...
            device_setup.add_connections(
                current_drive["device"],
                create_connection(
                    to_signal=f"{memory_mode}/cavity_drive_line", ports=current_drive["port"]
                ),
            )
            drive_index += 1
    ...
    if not readout_only:
        ...
        for qubit in qubits:
            ...
            if multiplex_drive_lines:
                ...
                for auxn in range(number_qubits):
                    device_setup.add_connections(
                        current_drive["device"],
                        create_connection(
                            to_signal=f"{qubit}/drive_line_aux{auxn}", ports=current_drive["port"]
                        ),
                    )
...
# generate a device setup from a list of Instrument objects
def generate_device_setup_mult(
    number_qubits: int = 6,
    qubit_port_pairs: dict[str, str] | None = None,
    qhub: list[dict[str]] | None = None,
    pqsc: list[dict[str]] | None = None,
    hdawg: list[dict[str]] | None = None,
    shfsg: list[dict[str]] | None = None,
    ...
    if not drive_only:
        for qubit in qubits:
            device_setup.add_connections(
                readout_acquire_lines[0]["device"],
                create_connection(to_signal=f"{qubit}/measure_QA_line", ports=f"QACHANNELS/0/OUTPUT"),
                create_connection(to_signal=f"{qubit}/acquire_QA_line", ports=f"QACHANNELS/0/INPUT"),
            )

    if not readout_only:
        for port in qubit_port_pairs:
            device_setup.add_connections(
                drive_lines[0]["device"],
                create_connection(to_signal=f"{qubit_port_pairs[port]}/drive_{port}_line", ports=f"SGCHANNELS/{port[1:]}/OUTPUT"),
            )
...
def generate_device_setup_qubits(
    ...
    device_setup = generate_device_setup(
        ...
        number_memory_modes=number_memory_modes,
        qhub=qhub,
        pqsc=pqsc,
        hdawg=hdawg,
        shfsg=shfsg,
...
def generate_device_setup_qubits_cr(
    number_qubits: int = 6,
    qhub: list[dict[str]] | None = None,
    pqsc: list[dict[str]] | None = None,
    hdawg: list[dict[str]] | None = None,
    ...
    device_setup = generate_device_setup_cr(
        number_qubits=number_qubits,
        qhub=qhub,
        pqsc=pqsc,
        hdawg=hdawg,
    ...
    dummy_qubit_parameters = generate_dummy_transmon_cr_parameters(
        number_of_qubits=number_qubits
    )

    qubits = [
        create_dummy_transmon_cr(
            it, base_parameters=dummy_qubit_parameters, device_setup=device_setup
        )
        for it in range(number_qubits)
    ]
...
def generate_device_setup_qubits_mult(
    number_qubits: int = 6,
    qubit_port_pairs: dict[str, str] | None = None,
    qhub: list[dict[str]] | None = None,
    pqsc: list[dict[str]] | None = None,
    ...
    device_setup = generate_device_setup_mult(
        number_qubits=number_qubits,
        qubit_port_pairs=qubit_port_pairs,
        qhub=qhub,
        pqsc=pqsc,
        hdawg=hdawg,
        shfsg=shfsg,
    ...
    dummy_qubit_parameters = generate_dummy_transmon_mult_parameters(
        number_of_qubits=number_qubits
    )

    qubits = [
        create_dummy_transmon_mult(
            it, base_parameters=dummy_qubit_parameters, device_setup=device_setup
        )
        for it in range(number_qubits)
    ]
