...
class SignalType_cr(Enum):
    ...
    DRIVE_AUX0 = "drive_aux0"
    DRIVE_AUX1 = "drive_aux1"
    DRIVE_AUX2 = "drive_aux2"
    DRIVE_AUX3 = "drive_aux3"
    DRIVE_AUX4 = "drive_aux4"
    DRIVE_AUX5 = "drive_aux5"
    # DRIVE_AUX6 = "drive_aux6"
    # DRIVE_AUX7 = "drive_aux7"
    # DRIVE_AUX8 = "drive_aux8"
    # DRIVE_AUX9 = "drive_aux9"


class QuantumElementSignalMap_cr(MutableMapping):
    def __init__(
        self,
        items: dict[str, str],
        key_validator: Callable[[str], SignalType_cr] | None = None,
    ) -> None:
        """A mapping between signal.
    ...
    def roles(self) -> dict[str, SignalType_cr]:
        """Roles of the signals."""
        roles = {}
        for sig in self._items:
            roles[sig] = SignalType_cr(sig)
        return roles


@classformatter
@dataclass(init=False, repr=True)
class QuantumElement_cr(ABC):
    """An abstract base class for quantum elements like Qubits or tunable couplers etc."""
    ...
    def __init__(
        ...
        self.uid = uuid.uuid4().hex if uid is None else uid
        if signals is None:
            self.signals = QuantumElementSignalMap_cr(
                {}, key_validator=self._coerce_signal_type
            )
        elif isinstance(signals, dict):
            sigs = {
    ...
    @staticmethod
    def _coerce_signal_type(name: str) -> SignalType_cr:
        """Coerce signal type.
        ...
        """
        try:
            return SignalType_cr(name)
        except ValueError:
            raise LabOneQException(
                f"Signal {name} is not one of {[enum.value for enum in SignalType_cr]}"
            ) from None
    ...
    def _from_logical_signal_group(
        ...
        lsg: LogicalSignalGroup,
        parameters: dict[str, Any] | None = None,
        signal_type_map: dict[SignalType_cr, List[str]] | None = None,
    ) -> "QuantumElement_cr":
        """Quantum Element from logical signal group.
        ...
        for name, signal in lsg.logical_signals.items():
            ...
            signal_map[signal_value] = cls._resolve_to_logical_signal_uid(signal)
        return cls(
            uid=uid, signals=QuantumElementSignalMap_cr(signal_map), parameters=parameters
        )

    @classmethod
    def load(cls, filename: Union[str, bytes, os.PathLike]) -> "QuantumElement_cr":
        """
        Loads a QuantumElement object from a JSON file.
    ...
    @classmethod
    def from_json(cls, filename: Union[str, bytes, os.PathLike]) -> "QuantumElement_cr":
        """Loads a QuantumElement object from a JSON file.
...
class QuantumElementSignalMap_mult(MutableMapping):
    def __init__(
        self,
        items: dict[str, str],
        key_validator: Callable[[str], SignalType_mult] | None = None,
    ) -> None:
        """A mapping between signal.
    ...
    def roles(self) -> dict[str, SignalType_mult]:
        """Roles of the signals."""
        roles = {}
        for sig in self._items:
            roles[sig] = SignalType_mult(sig)
        return roles


@classformatter
@dataclass(init=False, repr=True)
class QuantumElement_mult(ABC):
    """An abstract base class for quantum elements like Qubits or tunable couplers etc."""
    ...
    def __init__(
        ...
        self.uid = uuid.uuid4().hex if uid is None else uid
        if signals is None:
            self.signals = QuantumElementSignalMap_mult(
                {}, key_validator=self._coerce_signal_type
            )
        elif isinstance(signals, dict):
            sigs = {
    ...
    @staticmethod
    def _coerce_signal_type(name: str) -> SignalType_mult:
        """Coerce signal type.
        ...
        """
        try:
            return SignalType_mult(name)
        except ValueError:
            raise LabOneQException(
                f"Signal {name} is not one of {[enum.value for enum in SignalType_mult]}"
            ) from None
    ...
    def _from_logical_signal_group(
        ...
        lsg: LogicalSignalGroup,
        parameters: dict[str, Any] | None = None,
        signal_type_map: dict[SignalType_mult, List[str]] | None = None,
    ) -> "QuantumElement_mult":
        """Quantum Element from logical signal group.
        ...
        for name, signal in lsg.logical_signals.items():
            ...
            signal_map[signal_value] = cls._resolve_to_logical_signal_uid(signal)
        return cls(
            uid=uid, signals=QuantumElementSignalMap_mult(signal_map), parameters=parameters
        )

    @classmethod
    def load(cls, filename: Union[str, bytes, os.PathLike]) -> "QuantumElement_mult":
        """
        Loads a QuantumElement object from a JSON file.
    ...
    @classmethod
    def from_json(cls, filename: Union[str, bytes, os.PathLike]) -> "QuantumElement_mult":
        """Loads a QuantumElement object from a JSON file.
    ...
    def experiment_signals(
        ...
        with_types: bool = False,
        with_calibration: bool = False,
    ) -> Union[List[ExperimentSignal], List[Tuple[SignalType_mult, ExperimentSignal]]]:
        """Experiment signals of the quantum element.
